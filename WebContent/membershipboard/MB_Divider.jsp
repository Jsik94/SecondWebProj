...
<c:choose>
	<c:when test="${requestScope.WHERE =='Edit' }">	
		<c:set var="successMsg" value="수정이 완료되었습니다." />
		<c:set var="failMsg" value="수정이 실패하였습니다." />
		<c:set var="successUrl" value="/memboard/List.jsik?nowPage=${nowPage }&${query }" />
	</c:when>
	...
	<c:when test="${requestScope.WHERE =='Delete' }">
		<c:set var="successMsg" value="성공적으로 삭제되었습니다!" />
		<c:set var="failMsg" value="${failMsg }" />
		<c:set var="successUrl" value="/memboard/List.jsik?nowPage=${nowPage }&${query }" />
	</c:when>
	
	<c:when test="${requestScope.WHERE =='Write' }">
		<c:set var="successMsg" value="성공적으로 등록되었습니다!" />
		<c:set var="failMsg" value="등록에 실패했습니다." />
		<c:set var="successUrl" value="/memboard/List.jsik" />
	</c:when>
	<c:when test="${requestScope.WHERE =='View' }">
		<c:set var="failMsg" value="더이상 페이지가 없습니다." />
	</c:when>
